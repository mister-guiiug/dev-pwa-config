...
| --- | ------------------------------------------- | ---------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| 1   | Repli SPA `404.html`                        | [#145](https://github.com/mister-guiiug/dev-pwa-config/pull/145) | `spaFallbackPlugin()` + la copie dans `pwa-deploy.yml` — les quatre apps sont couvertes dès que `v3` suit              |
| 2   | Libellés en sept langues                    | [#146](https://github.com/mister-guiiug/dev-pwa-config/pull/146) | `es` `de` `it` `pt` `nl`, `labelsFor` (`pt-BR` → `pt`), `offlineMessage` sur `useActionGuard`                          |
| 3   | `react/card`                                | [#147](https://github.com/mister-guiiug/dev-pwa-config/pull/147) | `Card` + `CardHeader`, `[data-dwc="card"]`, fiche showroom                                                             |
| 4   | `id`                                        | [#148](https://github.com/mister-guiiug/dev-pwa-config/pull/148) | `createId`, `createUuid`, `isUuid` — et les deux copies internes importent d'ici                                       |
| 5   | Workflows réutilisables                     | [#149](https://github.com/mister-guiiug/dev-pwa-config/pull/149) | `cleanup-runs` appelable, `pwa-supabase-migrate`, `pwa-worker-deploy`, le keepalive enfin documenté                    |
| 6   | `react/app-header` + `react/page-container` | [#150](https://github.com/mister-guiiug/dev-pwa-config/pull/150) | l'en-tête, le conteneur, le rôle d'icône `back`, « Retour » en sept langues                                            |
| 7   | La couche auth                              | [#151](https://github.com/mister-guiiug/dev-pwa-config/pull/151) | `AuthProvider` / `useAuthContext`, `LoginForm`, `MfaChallenge` — non contrôlés, lus par `FormData`                     |
| 8   | `format`                                    | [#152](https://github.com/mister-guiiug/dev-pwa-config/pull/152) | `formatSigned`, `decimals`, `'auto'` sur le pourcentage, `{ never }` — et `null` n'est plus 1970                       |
| 9   | `pwa-bundle-budget`                         | [#153](https://github.com/mister-guiiug/dev-pwa-config/pull/153) | un bin, deux mesures, le budget dans `package.json`                                                                    |
