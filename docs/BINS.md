...
_Manuel du socle `@mister-guiiug/dev-pwa-config` — les sept commandes que le paquet installe, et ce qu’elles lisent. Retour au
[README](../README.md)._
...
| `pwa-bundle-budget` | Refuse un build qui dépasse `bundleBudget` lu dans `package.json`. Trois bornes, toutes facultatives : `totalGzipKb` (tout le JS gzip), `mainChunkKb` (le chunk principal, brut) et **`preloadGzipKb`** (le gzip que `index.html` RÉFÉRENCE, CSS compris — ce que le navigateur cherche avant le premier rendu). La troisième existe parce que les deux premières sont aveugles au chemin critique : mesuré sur miss-uwh, Sentry rangé dans `vendor` ou dans son propre morceau donne le MÊME total (444,3 contre 444,2 kB) pour 381,9 contre 227,2 kB préchargés. Un fichier référencé mais introuvable fait échouer au lieu de compter zéro. Ex. `"build": "vite build && pwa-bundle-budget"` — promu de miss-uwh et mister-qowa. `--ratchet` propose un budget resserré (mesure + 10 %) quand le build a maigri, `--write` l'écrit : un budget posé un jour de surpoids ne redescendait jamais                                                                                                                                                                      |
...
| `pwa-og-image`      | L'image de partage 1200×630 (`public/og-image.jpg`) : fond à la couleur du thème, icône, nom et accroche de l'app, adresse, et la capture en hauteur du manifeste dans un cadre de téléphone. Tout est lu dans l'app (catalogue, `<title>`, `theme-color` de `dist/index.html`, plus grande icône du dossier public). `pwaSeoPlugin` la trouve ensuite seul et pose `og:image` + `summary_large_image`. JPEG : hors du précache. Requiert Playwright et Chromium. Ex. `npx pwa-og-image --no-screenshot`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `pwa-typecheck-7`   | **Second avis** TypeScript 7, à côté de la 6 qui reste la référence. `typescript-eslint` refuse la 7.0 par une assertion à l'import — remplacer `typescript` par la 7 éteint ESLint pour TOUS les fichiers du dépôt ; la seule disposition tenable est la cohabitation par alias (`"typescript-7": "npm:typescript@~7.0.2"`). Deux gardes, chacune née d'une erreur mesurée : il TRAVERSE un `tsconfig.json` « solution » (`files: []` + `references`) jusqu'aux projets qui portent du code — vingt apps du parc sont dans ce cas, et les viser directement fait lire **zéro fichier** en sortant 0 ; et il ÉCHOUE quand le compte de fichiers lus est nul, parce qu'un contrôle qui ne lit rien et se déclare vert est pire que pas de contrôle. `--noEmit` par projet, jamais `tsc -b`, qui écrirait `.d.ts` et `.tsbuildinfo`. Des tsconfig passés en arguments l'emportent sur la découverte. Promu de miss-ticket, seule app à l'avoir demandée. Ex. `"type-check:7": "pwa-typecheck-7"`, avec `run-type-check-7: true` dans `pwa-ci.yml` (opt-in, NON bloquant) |
