...
| #   | Chantier            | Fait                                                                                                                                                                                                                  | Reste                                                                                                    |
| --- | ------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------- |
| 1   | Renovate            | Préréglage `renovate/default.json` + `renovate.yml` auto-hébergé dans le socle (#157) ; les dix-sept apps étendent le préréglage (PR « conformité »)                                                                  | Le secret `RENOVATE_TOKEN` sur le socle (propriétaire)                                                   |
| 2   | Installabilité      | ticket-pwa : manifeste lié sous le site, PNG 192/512 + maskable, `id`, `lang`, icône iOS ; lookhouse : PNG ; cim10, footcoach, puzzle : `id`, `lang: 'fr'`, maskable ; carbook : `id`, icône iOS ; family-map : `id`  | Captures pour lookhouse, qowa, supaboss, family-map                                                      |
| 3   | Dette d'adoption    | Règle de façade corrigée (#156) : 34 copies                                                                                                                                                                           | L'alignement des apps, après la 3.33.0 (PR #160, sur feu vert)                                           |
| 4   | `pwa-doctor`        | Bin publié (#158, #159)                                                                                                                                                                                               | Le poser en `postbuild --strict` app par app                                                             |
| 5   | Poids               | carbook : 432 → 146 kB de JS initial gzip (plus de chunk `charts` ni de fourre-tout `vendor`)                                                                                                                         | Budgets `bundleBudget` + `pwa-bundle-budget` en CI, avec la 3.33.0                                       |
| 6   | SEO et partage      | `pwaSeoPlugin` + balises Open Graph et canonique sur lookhouse, supaboss, qowa ; `themeColor` par schéma sur les onze apps qui ne l'avaient pas ; CSP sur lookhouse et qowa                                           | —                                                                                                        |
| 7   | Repli 404           | Livré par `v3` à la publication de la 3.33.0                                                                                                                                                                          | Vérifier après publication : `node scripts/probe-sites.mjs miss-badminton` doit dire `404→coquille`      |
| 8   | Dépôts hors gabarit | lookhouse, qowa, supaboss : Lighthouse, `cleanup-runs`, `.lighthouserc.json`, `.editorconfig`, `.nvmrc` ; supaboss : spec a11y ; puzzle : e2e en CI (spec critique réparée) ; carbook et molkky : keep-alive Supabase | Keep-alive sur uwh, doc, footcoach, lookhouse : il leur manque les secrets `VITE_SUPABASE_*`             |
| 9   | Locale figée        | 71 appels dans dix apps passent par `getDefaultLocale()` ; les ternaires d'i18n et les réglages TTS restent                                                                                                           | —                                                                                                        |
| 10  | Code mort           | 50 exports morts retirés, 6 fichiers supprimés, une centaine dé-exportés, dans quinze apps                                                                                                                            | 3 retraits trop larges rétablis (badminton `migrations`, carbook `IconActionButton`, ticket-pwa `theme`) |
...
Ce que la journée a appris à l'outillage : la règle de façade (§ 3), les citations hors `src/` (§ 10), et qu'un heredoc de ce poste mange les antislashs — les codemods s'écrivent dans des fichiers, pas en ligne de commande.
