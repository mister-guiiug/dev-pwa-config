...
| `pwa-supabase-migrate.yml`       | `deploy-functions`, `functions-flags`            | `SUPABASE_ACCESS_TOKEN`, `SUPABASE_DB_PASSWORD` ; `SUPABASE_PROJECT_ID` est un slot secret **nourri par `vars`** | échoue en nommant le secret absent — conditionner le job à `vars.VITE_SUPABASE_URL != ''` |
