...
Le relevé est sans appel : **129 fichiers recopiés dans les dix-sept apps, et
...
Relevé complet, dix-sept dépôts clonés côte à côte. **132 doublons avant, 129
après**, plus un en attente de publication (`geo` de `mister-family-map`). Le
chiffre bouge peu, et c'est le vrai résultat de ce passage : la campagne
butait sur un prérequis que personne n'avait vu.
...
| `mister-family-map` | `geo` — 14 fichiers ; PR dans `bac-sable`, publication à suivre |
