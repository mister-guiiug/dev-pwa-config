...
globalThis.SHOWROOM_I18N = {
  en: {
    ...
    'apps.p1':
      'Sixteen public repositories, every one of them a consumer of this package. This grid is not a list kept by hand in the page: it is generated from <code>apps-catalog.js</code> — the very file the applications import to show one another. Anything wrong here would be wrong in their footers.',
    'apps.note':
      '<strong>Domain and maturity are editorial</strong>, entered by hand: they state an intent, not a published version. <strong>Persistence is surveyed</strong> in each app’s code, and left blank where it was not — a filter that says “not surveyed” beats invented data. The badges are painted with each application’s real primary colour, never with a remote icon: this showroom makes no network request.',
