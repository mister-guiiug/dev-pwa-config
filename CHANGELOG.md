...
- 292041f: `pwaSeoPlugin` sert des **pages de contenu** et une **image de partage**, sans
  réglage.
...
- ebe574b: `ical` : des rappels `VALARM`, seulement sur demande. Un événement qui porte `alarms: [{ minutesBefore, description? }]` reçoit un rappel d'affichage par entrée, compté depuis son début, et un événement sans `alarms` n'en porte toujours aucun. Pour les exports dont le rappel est l'objet, comme les échéances de licence de miss-uwh.

## 6.15.1

### Patch Changes

- d969e88: `pwa-doctor` : un composant rangé dans un dossier de réglages compte pour l'écran qui le rend
...
- f3a1fed: `pwa-doctor` : l'accueil se reconnaît aussi à sa route, plus seulement à son nom de fichier
...
- aa93506: Pair `@sentry/react` : `^10.75.2 || ^11.0.0` — le socle accepte Sentry 11
...
- 4f5e7ef: `vite-pwa` : les fichiers échappent au repli de navigation du service worker
...
- 51b087a: `pwa-vice.yml` : l'audit de sécurité VICE, en second avis non bloquant
...
- b87d49e: Node 26.10.0 dans toute la CI, comme le `.nvmrc`
...
- 0642a83: `pwaSeoPlugin` : un contenu SERVI dans le point de montage, pour les robots qui n'exécutent pas le JavaScript
...
- 07a7d50: `pwaSeoPlugin` : données structurées `WebApplication`, et `lastmod` au plan de site
...
- 382ccfd: Actions GitHub : `actions/checkout` et `actions/setup-node` en `@v7`
...
- fc32125: `pwa-doctor` : un morceau hors précache ne doit pas porter d'empreinte
...
- f597c0c: Les types de `speech` suivent enfin son code
...
- 0e15c44: `speech` : laisser l'utilisateur choisir sa voix
...
- ce8200b: `speech` : la voix par DÉFAUT du système gagne dans sa langue
...
- 923b415: `FamilyApps` : deux props qui reprennent ce que les apps écrivaient en CSS.
...
- e0b0ccb: Node 26.9.0 partout dans la CI, et ESLint 10.11.0.
...
- 195fa07: `img-src` nomme l'origine de la famille — et l'icône de Mölkky maigrit de 1,28 Mo
...
- ee91f3f: Le clic qui répond, le morceau qui arrive avant, le repli qui se relit — trois
...
- 44abccc: `pwa-doctor` LIT le majeur du socle au lieu de le figer — il exigeait encore `@v4` depuis la 6.0.0.
  ...
  **LE DOCTEUR EST LE SOCLE**, et c'est ce qui rend la lecture juste. Il s'exécute depuis le `node_modules` de l'application : sa version EST celle dont l'application dépend. Une app sur `^4.21.3` lance le docteur 4 et doit référencer `@v4` ; une app sur `^6.0.0` lance le docteur 6 et doit référencer `@v6`. Il n'y a jamais eu besoin d'écrire le chiffre.
  ...
  ### Et une affirmation démentie par le navigateur
  
  `ANALYTICS_HOSTS` de `vite-csp` documentait l'hôte `eu-assets.i.posthog.com` en `script-src` comme une **précaution**, au motif que « quand `posthog-js` est installé en dépendance, tout est dans le bundle et rien n'est chargé de là ».
  
  C'est faux. Mesuré le 19/09/2026 sur le squelette **construit** et servi — le mode développement ne prouverait rien, `cspPlugin` y posant `'unsafe-inline'` :
  
  ```
  https://eu-assets.i.posthog.com/array/<clé>/config.js
  initiatorType: "script", responseStatus: 200
  ```
  
  `posthog-js` va chercher sa configuration distante à **chaque `init`**. Une CSP qui omettrait cet hôte couperait la mesure en silence — précisément le mode de panne que ce fichier décrit deux paragraphes plus haut. Aucun changement de comportement : l'hôte était là et il y reste. Ce qui change, c'est qu'on sait pourquoi.
...
- 45591c6: `initAnalytics` POSE `cookie_domain` au lieu de laisser gtag le deviner — le parc est servi sous un suffixe public.
...
- 6553918: `vitest-setup` rétablit `URL.createObjectURL`, que le couple Vitest 5 / jsdom 30.1 a cassée.
...
- db3c27a: Le gabarit `templates/index.html` ne porte plus les marqueurs `__ANALYTICS_HEAD__` / `__ANALYTICS_BODY__`, et `buildAnalyticsHtmlFragments` est marquée obsolète.
...
- 4e3ca2d: **`react/consent-banner` : la pièce qui DEMANDE le consentement.**
...
- d106369: **L'audit SCA suit `server-dir` : les vingt-cinq alertes restantes du parc
...
- 668300c: **Le bandeau de mise à jour dit « Mettre à jour », et son report annonce sa
  durée.**
...
- ebcdf97: **`/qr` repose sur `uqr` et non plus sur `qrcode` — et `qrToDataUrl` rend un SVG.**
...
- 8d94a0f: **ESLint ignore `coverage/`.**
...
- c6a6905: **`run-npm-audit` passe à `true` par défaut : le parc n'avait plus aucun canal
  ...
  **Pourquoi ce défaut-ci, quand `run-coverage` reste opt-in.** Le changeset
  ...
  changement de workflow réutilisable n'atteint aucun des dix-neuf consommateurs
  tant que rien n'est publié.
- ba3f6b3: **`pwa-ci.yml` accepte `run-coverage`, et refuse de le faire à moitié.**
...
- 59ce941: **`UpdatePromptBanner` accepte `checkEvery`.**
...
- 55f9855: **`pwa-icons` produit `apple-touch-icon.png`.**
...
- dcb7ceb: **`pwa-icons` créait ses fichiers mais pas son dossier.**
...
- 208afa3: `vitest-setup` rend aux tests le `localStorage` de jsdom, que Node 26 masquait.
...
- 16dabed: `AnimationEvent` rétabli dans `vitest-setup`, et quatre plafonds levés.
...
- 24728d6: Node 26.2.0 partout dans la CI, et Prettier 3.9.6.
...
- d9869f9: **`pwa-doctor` gagne un droit de réponse, et `secrets: inherit` devient un
...
- **`mister-settle` entre au catalogue — la vingtième app de la famille.**

  Partage de dépenses entre proches, née le 07/09/2026 de `create-lg-pwa-app` et
  livrée en quatorze lots dans la journée. Elle consomme **58 sous-chemins**, ce
  qu'aucune autre app n'approche : première adoptante de `sync-queue`, `csv`,
  `xlsx`, `image` et `react/use-action-guard` **ensemble** — l'hors-ligne de
  l'ADR 0015 du squelette, joué de bout en bout, file de création idempotente
  comprise.

  Ce qui entre avec elle : sa fiche (`outils`, backend `supabase`), son port de
  développement (5209), sa palette **relevée** dans son `src/index.css`, et le
  relevé d'adoption refait sur les vingt dépôts. Les apps la verront dans leur
  `<FamilyApps>` à leur prochaine montée de version.

  Deux enseignements pour le paquet, tirés de sa construction :
  - `createSupabaseClientFactory` ne voit pas `import.meta.env` du consommateur —
    sans l'option `env`, une app publiée annonce « configuration manquante » ;
  - le contrôle « liens-famille » de `pwa-doctor` lit un `/*` écrit **dans une
    chaîne** comme un commentaire de bloc, et avale le `<Routes>` qui suit : une
    app qui route en `matchPath('/e/:id/*')` se voyait reprocher des liens
    absents.
...
- d645f81: **Le bandeau d'installation ne s'affichait jamais sur iPhone, et un « Plus
...
- 2406d0a: **`pwa-bindings` ne posait rien sur Windows — la seule plateforme pour laquelle
  il existe.**
...
- 02ae951: **`pwa-bindings` : les binaires natifs du poste, aux versions du lockfile.**
